...
def start(message):
    ...
    markup.add(btn1, btn2, btn3, btn4, btn5)  # и добавляем их к нашей разметки
...
def mess(message):
    ...
    if get_message_bot == "привет":  # если пользователь нажал кнопку или написал "Привет", то запускается этот скрипт
        ...
        pic1 = open('library_with_photo/Diploma.png', 'rb')
        bot.send_photo(message.chat.id, pic1)
        bot.send_message(message.chat.id, f'Смотрите какой у меня красивый Диплом XD')

        pic2 = open('library_with_photo/report.png', 'rb')
        pic3 = open('library_with_photo/oficial.png', 'rb')
        pic4 = open('library_with_photo/price.png', 'rb')
